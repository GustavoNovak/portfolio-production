...
if(isset($_POST['faturamento']) && !empty($_POST['faturamento'])){
	...
	try{
		...
		for($i=0;$i<=11;$i++){
			...
			$sql = "SELECT * FROM registro_faturamento WHERE id_usuario='".$_SESSION['id']."' AND cnpj='".utf8_encode($_SESSION['cnpj'])."' AND data='".$data."'";
			$sql = $db->query($sql);

			if($sql->rowCount() == 1){
				$sql = "UPDATE registro_faturamento SET faturamento='".$valor."' WHERE id_usuario='".$_SESSION['id']."' AND cnpj='".utf8_encode($_SESSION['cnpj'])."' AND data='".$data."'";
				$sql = $db->query($sql);	
			} else {
				$sql = "INSERT INTO registro_faturamento SET  id_usuario='".$_SESSION['id']."', cnpj='".utf8_encode($_SESSION['cnpj'])."', faturamento='".$valor."', data='".$data."'";
				$sql = $db->query($sql);					
			}
		} 
			...
			<form name="form" method="post" action="resultado.php">
				<?php
					...
					foreach($_SESSION['categorias'] as $categoria){
						...
						echo 	"Anexo ".$categoria['anexo']." - ".$_SESSION['nome_atividade'][$i].":"."<br/><br/>";
						echo "R$<input type='text' name='faturamento_categoria[]' data-thousands='.'' data-decimal=',' class='dinheiroo' autocomplete='off' required/>";
						echo "</div><br/>";
