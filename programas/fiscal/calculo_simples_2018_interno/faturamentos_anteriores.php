...
if(isset($_POST['folha']) && !empty($_POST['folha'])){
	...
	try{
		...
		for($i=0;$i<=11;$i++){
			...
			$sql = "SELECT * FROM registro_folha WHERE id_usuario='".$_SESSION['id']."' AND cod_empresa='".$_SESSION['cod_empresa']."' AND data='".$data."'";
			$sql = $db->query($sql);

			if($sql->rowCount() == 1){
				$sql = "UPDATE registro_folha SET folha='".$valor."' WHERE id_usuario='".$_SESSION['id']."' AND cod_empresa='".$_SESSION['cod_empresa']."' AND data='".$data."'";
				$sql = $db->query($sql);	
			} else {
				$sql = "INSERT INTO registro_folha SET  id_usuario='".$_SESSION['id']."', cod_empresa='".$_SESSION['cod_empresa']."', folha='".$valor."', data='".$data."'";
				$sql = $db->query($sql);					
			}
		} 
	} catch(PDOexception $e){
...
<section id="banner">
	...
	try{
		$db = new PDO($dsn,$dbuser,$dbpass);
		$sql = "SELECT * FROM registro_faturamento WHERE id_usuario='".$_SESSION['id']."' AND cod_empresa='".$_SESSION['cod_empresa']."'";
		$sql = $db->query($sql);
