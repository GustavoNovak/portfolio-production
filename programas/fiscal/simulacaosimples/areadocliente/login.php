...
if(isset($_SESSION['id_usuario_areadocliente']) && !empty($_SESSION['id_usuario_areadocliente']) && isset($_SESSION['id_empresa_areadocliente']) && !empty($_SESSION['id_empresa_areadocliente'])){
	...
	unset($_SESSION['id_empresa_areadocliente']);
}
...
if(isset($_POST['usuario']) && !empty($_POST['usuario']) && isset($_POST['senha']) && !empty($_POST['senha'])){
	...
	$sql = $pdo->prepare("SELECT * FROM usuarios_areadocliente WHERE login = :usuario AND senha = :senha ");
	$sql->bindValue(":usuario",addslashes($_POST['usuario']));
	$sql->bindValue(":senha",md5(addslashes($_POST['senha'])));
	$sql->execute();
	if($sql->rowCount() > 0){
		$usuario = $sql->fetch();
		$_SESSION['id_usuario_areadocliente'] = $usuario['id'];
		...
		if($sql->rowCount() > 0){
			if($sql->rowCount() > 1){
				...
				$empresa = $sql-> fetch();
				$_SESSION['id_empresa_areadocliente'] = $empresa['id_empresa'];
